...
async function createNode (self, farmID, resources, location, countryID, cityID, role, publicConfig, callback) {
  try {
  ...
  const parsedRole = self.api.createType('Role', role)
  const parsedPublicConfig = self.api.createType('PublicConfig', publicConfig)

  const create = self.api.tx.tfgridModule.createNode(farmID, parsedResources, parsedLocation, countryID, cityID, parsedRole, parsedPublicConfig)
...
async function updateNode (self, nodeID, farmID, resources, location, countryID, cityID, role, publicConfig, callback) {
  ...
  const create = self.api.tx.tfgridModule.updateNode(farmID, parsedResources, parsedLocation, countryID, cityID, parsedRole, parsedPublicConfig)
  const nonce = await self.api.rpc.system.accountNextIndex(self.address)

...
async function getNode (self, id) {
  ...
  // decode resources
  const { resources: { totalResources, usedResources } } = res
  for (const key of Object.keys(totalResources)) {
    totalResources[key] = +totalResources[key]
    usedResources[key] = +usedResources[key]
  }
  ...
  if (country) {
    location.country = hex2a(country)
  }

  if (city) {
    location.city = hex2a(city)
  }

  const { publicConfig } = res
  if (publicConfig) {
    const { ip4, ip6 } = publicConfig
    if (ip4) {
      publicConfig.ip4.ip = hex2a(ip4.ip)
      publicConfig.ip4.gw = hex2a(ip4.gw)
    }
    if (ip6) {
      publicConfig.ip6.ip = hex2a(ip6.ip)
      publicConfig.ip6.gw = hex2a(ip6.gw)
    }
  }

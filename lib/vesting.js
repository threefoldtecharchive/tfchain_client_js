...
async function reportFailedTransaction (transaction, reason) {
  const create = this.api.tx.vestingValidatorModule.reportFailedTransaction(transaction, reason)
  const nonce = await this.api.rpc.system.accountNextIndex(this.address)
...
async function listVestingValidators () {
  const validators = await this.api.query.vestingValidatorModule.validators()

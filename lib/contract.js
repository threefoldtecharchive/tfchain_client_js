...
async function createNodeContract (self, nodeID, hash, data, numberOfPublicIPs, solutionProviderID, callback) {
...
async function createNameContract (self, name, solutionProviderID, callback) {
  const create = self.api.tx.smartContractModule.createNameContract(name, solutionProviderID)
  const nonce = await self.api.rpc.system.accountNextIndex(self.address)

...
async function listDeployments (self) {
  const deployments = await self.api.query.smartContractModule.deployments.entries(self.address)
  return deployments.map((d)=> d.toJSON())
}
...
async function listContracts () {
  const contracts = await self.api.query.smartContractModule.contracts.entries(self.address)
  return contracts.map((d)=> d.toJSON())
}

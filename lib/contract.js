...
async function createNodeContract (self, nodeID, data, hash, numberOfPublicIPs, callback) {
  const create = self.api.tx.smartContractModule.createNodeContract(nodeID, data, hash, numberOfPublicIPs)
  const nonce = await self.api.rpc.system.accountNextIndex(self.address)

  return create.signAndSend(self.key || self.address, { nonce, signer: self.extSigner.signer }, callback)
}

async function updateNodeContract (self, contractID, data, hash, callback) {
  const update = self.api.tx.smartContractModule.updateNodeContract(contractID, data, hash)
  const nonce = await self.api.rpc.system.accountNextIndex(self.address)

  return update.signAndSend(self.key || self.address, { nonce, signer: self.extSigner.signer }, callback)
}
...
async function createRentContract (self, nodeId, callback) {
  const create = self.api.tx.smartContractModule.createRentContract(nodeId)
  const nonce = await self.api.rpc.system.accountNextIndex(self.address)
...
async function cancelContract (self, contractID, callback) {
  ...
  return cancel.signAndSend(self.key || self.address, { nonce, signer: self.extSigner.signer }, callback)
}

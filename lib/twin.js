...
async function deleteTwinEntity (self, twinID, entityID, callback) {
  ...
  return remove.signAndSend(self.key || self.address, { nonce, signer: self.extSigner.signer }, callback)
}
...
async function deleteTwin (self, id, callback) {
  ...
  return self.api.tx.tfgridModule
    .deleteTwin(id)
    .signAndSend(self.key || self.address, { nonce, signer: self.extSigner.signer }, callback)
}
